...
/**
 * Unified compiler page that supports multiple languages.
 * The language is determined by the :lang route parameter.
 *
 * To add a new language, add an entry to languageConfigs.js — no
 * changes needed here.
 */
export default function Compiler() {
  ...
  return <CompilerPage config={config} key={lang} />;
}

/**
 * Inner component that renders the compiler UI for a specific language.
 * Separated so that `key={lang}` resets state when switching languages.
 */
function CompilerPage({ config }) {
  ...
  const {
    ...
    downloadOutput,
  } = useCodeExecution(config);

  const { EditorComponent, title, inputHint, inputFunctionName } = config;
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-2 space-y-6">
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-xl shadow-2xl overflow-hidden">
              ...
              <div className="p-0">
                <EditorComponent value={code} onChange={handleCodeChange} />
              </div>
            </div>
